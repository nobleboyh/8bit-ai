...
- [ ] Create `App.tsx` layout shell
  - [ ] Header with "PIXELFORGE" title and "8-BIT AVATAR GENERATOR" subtitle
  - [ ] Three numbered sections: "01 // CONFIG", "02 // PROMPT", "03 // AVATAR"
  - [ ] Section container with 3px solid border, 24px padding, `--border` colour
  - [ ] Container max-width 720px, centered
  - [ ] Section gap: 24px
  - [ ] ThemeToggle mounted in header area
  - [ ] Wire up ApiKeyInput + ProviderSelect in CONFIG section
- [ ] Create `ControlPanel` component
  - [ ] File: `src/components/ControlPanel/ControlPanel.tsx`
  - [ ] CSS module: `src/components/ControlPanel/ControlPanel.module.css`
  - [ ] Test: `src/components/ControlPanel/ControlPanel.test.tsx`
  - [ ] Textarea with placeholder: "Describe your character... e.g. A wise old wizard with a sparkling staff"
  - [ ] "FORGE" button with `.btn-primary` styling
  - [ ] Disabled state for FORGE button (opacity 0.4, not-allowed)
  - [ ] FORGE button disabled when textarea empty or API key missing
  - [ ] Cmd/Ctrl+Enter keyboard shortcut triggers generate
  - [ ] 6 example chips: Wizard, Knight, Robot, Astronaut, Ninja, Elf
  - [ ] Chip hover: accent border + text highlight
  - [ ] Chip click fills textarea with preset prompt text
  - [ ] Action rows: `.generate-row`, `.examples-row` for layout
  - [ ] Generate row: textarea + FORGE button side by side on desktop
- [ ] Create example chip presets
...
- [ ] Create empty viewer state
  - [ ] Viewer container with min-height 320px (desktop)
  - [ ] Black background, 4px border, 8px 8px offset shadow (`--shadow`)
  - [ ] Centered empty state: large pixel icon U+25A3 in `--muted` colour
  - [ ] Instructional text: "ENTER A PROMPT & FORGE YOUR PIXEL" in `--font-display`, `--muted` colour
  - [ ] `image-rendering: pixelated` on container
- [ ] Implement responsive design
  - [ ] `@media (max-width: 600px)`: section padding → 16px, action rows stack vertical, viewer min-height → 240px
  - [ ] `@media (max-width: 360px)`: section padding → 8px, smaller chip text
  - [ ] Use `clamp()` for fluid font sizing
- [ ] Write tests
  - [ ] ControlPanel: renders textarea, FORGE button, 6 chips
  - [ ] ControlPanel: clicking chip fills textarea
  - [ ] ControlPanel: FORGE button disabled when textarea empty
  - [ ] ControlPanel: Cmd/Ctrl+Enter triggers generate callback
  - [ ] Viewer: shows empty state with icon and instructional text
  - [ ] Section containers: correct border, padding, gap styles
